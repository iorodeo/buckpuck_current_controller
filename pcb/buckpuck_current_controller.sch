EESchema Schematic File Version 2  date Tue 07 Jun 2011 09:12:28 PM PDT
LIBS:power
LIBS:device
LIBS:transistors
LIBS:conn
LIBS:regul
LIBS:74xx
LIBS:cmos4000
LIBS:adc-dac
LIBS:memory
LIBS:xilinx
LIBS:special
LIBS:microcontrollers
LIBS:dsp
LIBS:microchip
LIBS:analog_switches
LIBS:motorola
LIBS:texas
LIBS:intel
LIBS:audio
LIBS:interface
LIBS:digital-audio
LIBS:philips
LIBS:display
LIBS:cypress
LIBS:siliconi
LIBS:opto
LIBS:atmel
LIBS:contrib
LIBS:valves
LIBS:buckpuck_current_controller
LIBS:buckpuck_current_controller-cache
EELAYER 24  0
EELAYER END
$Descr A4 11700 8267
Sheet 1 5
Title "Buckpuck Current Controller"
Date "8 jun 2011"
Rev "2.0"
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
Wire Wire Line
	4600 3800 4600 3700
Wire Wire Line
	4600 3700 4350 3700
Wire Wire Line
	9750 2100 9950 2100
Wire Wire Line
	8250 1900 7850 1900
Connection ~ 1600 6500
Wire Wire Line
	1600 6500 1700 6500
Connection ~ 1600 6300
Wire Wire Line
	1600 6300 1700 6300
Wire Wire Line
	1600 6700 1600 6100
Wire Wire Line
	1600 6100 1700 6100
Wire Wire Line
	2500 6200 2600 6200
Wire Wire Line
	2500 6400 2600 6400
Wire Wire Line
	2500 6600 2600 6600
Connection ~ 4600 5350
Wire Wire Line
	4600 5350 4700 5350
Wire Wire Line
	4700 5150 4600 5150
Wire Wire Line
	4600 5150 4600 5850
Wire Wire Line
	2650 4800 2250 4800
Wire Wire Line
	5050 5650 5050 5550
Wire Wire Line
	5050 5550 5600 5550
Connection ~ 7050 5450
Wire Wire Line
	7050 5450 6600 5450
Connection ~ 7350 4600
Wire Wire Line
	7350 4600 7050 4600
Wire Wire Line
	5600 5750 5300 5750
Wire Wire Line
	5600 5450 5300 5450
Wire Wire Line
	5600 5150 5300 5150
Wire Wire Line
	5600 4950 5300 4950
Wire Wire Line
	6600 5050 6700 5050
Wire Wire Line
	6600 5650 6700 5650
Wire Wire Line
	6600 5850 6700 5850
Wire Wire Line
	2650 4400 2050 4400
Wire Wire Line
	4600 1800 2350 1800
Wire Wire Line
	2350 1800 2350 1900
Wire Wire Line
	2350 1900 2500 1900
Wire Wire Line
	2650 5750 3150 5750
Wire Wire Line
	2650 5450 2700 5450
Wire Wire Line
	2700 5450 2700 5300
Connection ~ 4650 6700
Wire Wire Line
	4650 6700 4850 6700
Wire Wire Line
	4650 7350 4650 6400
Wire Wire Line
	4650 6400 4850 6400
Wire Wire Line
	6550 1650 6550 1800
Wire Wire Line
	5750 2300 5750 2450
Connection ~ 5750 1800
Wire Wire Line
	5750 1900 5750 1800
Wire Wire Line
	5000 2100 5000 2250
Wire Wire Line
	5900 3400 5750 3400
Wire Wire Line
	5750 3400 5750 3250
Wire Wire Line
	5450 3700 5450 3900
Connection ~ 6600 2950
Wire Wire Line
	6600 2950 6600 3400
Wire Wire Line
	6600 3400 6400 3400
Wire Wire Line
	6800 3050 6800 2950
Wire Wire Line
	6800 2950 6450 2950
Wire Wire Line
	5650 2950 5350 2950
Wire Wire Line
	2650 4500 2050 4500
Wire Wire Line
	2650 4700 2050 4700
Wire Wire Line
	2650 4900 2250 4900
Wire Wire Line
	4350 4000 4550 4000
Wire Wire Line
	4350 4200 4550 4200
Wire Wire Line
	4350 4400 4550 4400
Wire Wire Line
	4350 4600 4550 4600
Wire Wire Line
	3700 1800 3700 1600
Connection ~ 3450 1800
Wire Wire Line
	3450 2300 3450 2900
Wire Wire Line
	2950 2900 2950 2750
Wire Wire Line
	3200 3050 3200 2900
Connection ~ 3200 2900
Wire Wire Line
	3450 1900 3450 1500
Wire Wire Line
	4350 4700 4550 4700
Wire Wire Line
	4350 4500 4550 4500
Wire Wire Line
	4350 4300 4550 4300
Wire Wire Line
	4350 4100 4550 4100
Wire Wire Line
	2650 4300 2050 4300
Wire Wire Line
	2650 4100 2050 4100
Wire Wire Line
	2650 4600 2050 4600
Wire Wire Line
	2650 4000 2050 4000
Wire Wire Line
	2650 4200 2050 4200
Wire Wire Line
	2650 3900 1550 3900
Wire Wire Line
	1550 3900 1550 4000
Wire Wire Line
	2650 3800 2250 3800
Wire Wire Line
	6800 3450 6800 3600
Wire Wire Line
	4350 3600 4450 3600
Wire Wire Line
	3150 5650 2650 5650
Wire Wire Line
	4450 3600 4450 2950
Wire Wire Line
	5700 3850 5700 3900
Wire Wire Line
	5700 3900 4350 3900
Connection ~ 5450 3900
Connection ~ 3700 1800
Wire Wire Line
	6550 1800 5400 1800
Wire Wire Line
	4250 2300 4250 2450
Wire Wire Line
	4250 1800 4250 1900
Connection ~ 4250 1800
Wire Wire Line
	4850 6550 4650 6550
Connection ~ 4650 6550
Wire Wire Line
	4650 6850 4850 6850
Connection ~ 4650 6850
Wire Wire Line
	2650 5550 3550 5550
Wire Wire Line
	3550 5550 3550 5600
Wire Wire Line
	2500 2100 2350 2100
Wire Wire Line
	2350 2100 2350 2900
Wire Wire Line
	2350 2900 3450 2900
Connection ~ 2950 2900
Wire Wire Line
	2650 5000 2250 5000
Wire Wire Line
	6600 5350 6700 5350
Wire Wire Line
	6600 5750 6700 5750
Wire Wire Line
	6600 5150 6700 5150
Wire Wire Line
	6600 4950 6700 4950
Wire Wire Line
	5600 5050 5300 5050
Wire Wire Line
	5600 5350 5300 5350
Wire Wire Line
	5600 5650 5300 5650
Wire Wire Line
	5600 5850 5300 5850
Wire Wire Line
	7350 4700 7350 4500
Wire Wire Line
	6600 5550 7050 5550
Wire Wire Line
	7050 5550 7050 4600
Wire Wire Line
	6600 5250 7050 5250
Connection ~ 7050 5250
Wire Wire Line
	7350 5100 7350 5250
Wire Wire Line
	4350 5000 4550 5000
Wire Wire Line
	5600 5250 5300 5250
Wire Wire Line
	4600 5250 4700 5250
Connection ~ 4600 5250
Wire Wire Line
	4600 5450 4700 5450
Connection ~ 4600 5450
Wire Wire Line
	2500 6500 2600 6500
Wire Wire Line
	2500 6300 2600 6300
Wire Wire Line
	2500 6100 2600 6100
Wire Wire Line
	1700 6200 1600 6200
Connection ~ 1600 6200
Wire Wire Line
	1700 6400 1600 6400
Connection ~ 1600 6400
Wire Wire Line
	1700 6600 1600 6600
Connection ~ 1600 6600
Wire Wire Line
	9750 1700 9950 1700
Wire Wire Line
	9750 2250 9950 2250
Wire Wire Line
	9750 3550 9950 3550
Wire Wire Line
	9750 3000 9950 3000
Wire Wire Line
	8250 3200 7850 3200
Wire Wire Line
	9750 3400 9950 3400
Wire Wire Line
	9750 4700 9950 4700
Wire Wire Line
	8250 4500 7850 4500
Wire Wire Line
	9750 4300 9950 4300
Wire Wire Line
	9750 4850 9950 4850
Wire Wire Line
	9750 6200 9950 6200
Wire Wire Line
	9750 5650 9950 5650
Wire Wire Line
	8250 5850 7850 5850
Wire Wire Line
	9750 6050 9950 6050
Text Label 9950 6200 0    60   ~ 0
CurrentSenseD
Text Label 9950 6050 0    60   ~ 0
PotentiometerD
Text Label 9950 4850 0    60   ~ 0
CurrentSenseC
Text Label 9950 4700 0    60   ~ 0
PotentiometerC
Text Label 9950 3550 0    60   ~ 0
CurrentSenseB
Text Label 9950 3400 0    60   ~ 0
PotentiometerB
Text Label 9950 2250 0    60   ~ 0
CurrentSenseA
Text Label 9950 2100 0    60   ~ 0
PotentiometerA
Text Label 9950 5650 0    60   ~ 0
P3A
Text Label 9950 4300 0    60   ~ 0
P2A
Text Label 9950 3000 0    60   ~ 0
P1A
Text Label 9950 1700 0    60   ~ 0
P0A
Text Label 7850 5850 0    60   ~ 0
P3W
Text Label 7850 4500 0    60   ~ 0
P2W
Text Label 7850 3200 0    60   ~ 0
P1W
Text Label 7850 1900 0    60   ~ 0
P0W
$Sheet
S 8250 5400 1500 950 
U 4DEEDDA4
F0 "ChannelD" 60
F1 "single_buckpuck_channel.sch" 60
F2 "PotentiometerInput+" O R 9750 5650 60 
F3 "PotentiometerInputWiper" I L 8250 5850 60 
F4 "PotentiometerOutput" O R 9750 6050 60 
F5 "CurrentSense" O R 9750 6200 60 
$EndSheet
$Sheet
S 8250 4050 1500 950 
U 4DEEDD9B
F0 "ChannelC" 60
F1 "single_buckpuck_channel.sch" 60
F2 "PotentiometerInput+" O R 9750 4300 60 
F3 "PotentiometerInputWiper" I L 8250 4500 60 
F4 "PotentiometerOutput" O R 9750 4700 60 
F5 "CurrentSense" O R 9750 4850 60 
$EndSheet
$Sheet
S 8250 2750 1500 950 
U 4DEEDD8F
F0 "ChannelB" 60
F1 "single_buckpuck_channel.sch" 60
F2 "PotentiometerInput+" O R 9750 3000 60 
F3 "PotentiometerInputWiper" I L 8250 3200 60 
F4 "PotentiometerOutput" O R 9750 3400 60 
F5 "CurrentSense" O R 9750 3550 60 
$EndSheet
$Sheet
S 8250 1450 1500 950 
U 4DEEDD29
F0 "ChannelA" 60
F1 "single_buckpuck_channel.sch" 60
F2 "PotentiometerInput+" O R 9750 1700 60 
F3 "PotentiometerInputWiper" I L 8250 1900 60 
F4 "PotentiometerOutput" O R 9750 2100 60 
F5 "CurrentSense" O R 9750 2250 60 
$EndSheet
$Comp
L GND #PWR01
U 1 1 4DEEDCCB
P 1600 6700
F 0 "#PWR01" H 1600 6700 30  0001 C CNN
F 1 "GND" H 1600 6630 30  0001 C CNN
	1    1600 6700
	1    0    0    -1  
$EndComp
Text Label 2600 6100 0    60   ~ 0
SwitchF
Text Label 2600 6200 0    60   ~ 0
SwitchE
Text Label 2600 6300 0    60   ~ 0
SwitchD
Text Label 2600 6400 0    60   ~ 0
SwitchC
Text Label 2600 6500 0    60   ~ 0
SwitchB
Text Label 2600 6600 0    60   ~ 0
SwitchA
$Comp
L CONN_6X2 P4
U 1 1 4DEEDC47
P 2100 6350
F 0 "P4" H 2100 6700 60  0000 C CNN
F 1 "Switches" V 2100 6350 60  0000 C CNN
	1    2100 6350
	-1   0    0    1   
$EndComp
Text Label 4700 5450 0    60   ~ 0
P3B
Text Label 4700 5350 0    60   ~ 0
P2B
Text Label 4700 5250 0    60   ~ 0
P1B
Text Label 4700 5150 0    60   ~ 0
P0B
$Comp
L GND #PWR02
U 1 1 4DEEC339
P 4600 5850
F 0 "#PWR02" H 4600 5850 30  0001 C CNN
F 1 "GND" H 4600 5780 30  0001 C CNN
	1    4600 5850
	1    0    0    -1  
$EndComp
Text Label 5300 5250 0    60   ~ 0
CS
$Comp
L GND #PWR03
U 1 1 4DEEBDF5
P 7350 5250
F 0 "#PWR03" H 7350 5250 30  0001 C CNN
F 1 "GND" H 7350 5180 30  0001 C CNN
	1    7350 5250
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR04
U 1 1 4DEEBDE2
P 5050 5650
F 0 "#PWR04" H 5050 5650 30  0001 C CNN
F 1 "GND" H 5050 5580 30  0001 C CNN
	1    5050 5650
	1    0    0    -1  
$EndComp
$Comp
L +5V #PWR05
U 1 1 4DEEBDBE
P 7350 4500
F 0 "#PWR05" H 7350 4590 20  0001 C CNN
F 1 "+5V" H 7350 4590 30  0000 C CNN
	1    7350 4500
	1    0    0    -1  
$EndComp
$Comp
L C C5
U 1 1 4DEEBDAE
P 7350 4900
F 0 "C5" H 7400 5000 50  0000 L CNN
F 1 "0.1uF" H 7400 4800 50  0000 L CNN
	1    7350 4900
	1    0    0    -1  
$EndComp
Text Label 5300 5850 0    60   ~ 0
P1A
Text Label 5300 5750 0    60   ~ 0
P1W
Text Label 5300 5650 0    60   ~ 0
P1B
Text Label 5300 5450 0    60   ~ 0
MOSI
Text Label 5300 5350 0    60   ~ 0
SCK
Text Label 5300 5150 0    60   ~ 0
P3B
Text Label 5300 5050 0    60   ~ 0
P3W
Text Label 5300 4950 0    60   ~ 0
P3A
Text Label 6700 5850 0    60   ~ 0
P0A
Text Label 6700 5750 0    60   ~ 0
P0W
Text Label 6700 5650 0    60   ~ 0
P0B
Text Label 6700 5150 0    60   ~ 0
P2B
Text Label 6700 5050 0    60   ~ 0
P2W
Text Label 6700 4950 0    60   ~ 0
P2A
Text Label 6700 5350 0    60   ~ 0
MISO
$Comp
L MCP434X/436X U3
U 1 1 4DEEB6BE
P 6100 5400
F 0 "U3" H 6100 4850 60  0000 C CNN
F 1 "MCP434X/436X" H 6100 5950 60  0000 C CNN
	1    6100 5400
	1    0    0    -1  
$EndComp
Text Label 2050 4000 0    60   ~ 0
SwitchF
Text Label 2050 4100 0    60   ~ 0
SwitchE
Text Label 2050 4200 0    60   ~ 0
SwitchD
Text Label 2050 4300 0    60   ~ 0
SwitchC
Text Label 2050 4400 0    60   ~ 0
SwitchB
Text Label 2050 4500 0    60   ~ 0
SwitchA
Text Label 2250 4800 0    60   ~ 0
CS
Text Label 4550 5000 0    60   ~ 0
SCK
Text Label 2250 4900 0    60   ~ 0
MOSI
Text Label 2250 5000 0    60   ~ 0
MISO
Text Label 2700 5750 0    60   ~ 0
DisplayTX
Text Label 2700 5650 0    60   ~ 0
DisplayRX
$Comp
L GND #PWR06
U 1 1 4DD6F401
P 4650 7350
F 0 "#PWR06" H 4650 7350 30  0001 C CNN
F 1 "GND" H 4650 7280 30  0001 C CNN
	1    4650 7350
	1    0    0    -1  
$EndComp
$Comp
L CONN_1 P8
U 1 1 4DD6F3EB
P 5000 6850
F 0 "P8" H 5080 6850 40  0000 L CNN
F 1 "CONN_1" H 5000 6905 30  0001 C CNN
	1    5000 6850
	1    0    0    -1  
$EndComp
$Comp
L CONN_1 P7
U 1 1 4DD6F3E8
P 5000 6700
F 0 "P7" H 5080 6700 40  0000 L CNN
F 1 "CONN_1" H 5000 6755 30  0001 C CNN
	1    5000 6700
	1    0    0    -1  
$EndComp
$Comp
L CONN_1 P6
U 1 1 4DD6F3E5
P 5000 6550
F 0 "P6" H 5080 6550 40  0000 L CNN
F 1 "CONN_1" H 5000 6605 30  0001 C CNN
	1    5000 6550
	1    0    0    -1  
$EndComp
$Comp
L CONN_1 P5
U 1 1 4DD6F3DD
P 5000 6400
F 0 "P5" H 5080 6400 40  0000 L CNN
F 1 "CONN_1" H 5000 6455 30  0001 C CNN
	1    5000 6400
	1    0    0    -1  
$EndComp
$Comp
L VDD #PWR07
U 1 1 4DD30F8E
P 6550 1650
F 0 "#PWR07" H 6550 1750 30  0001 C CNN
F 1 "VDD" H 6550 1760 30  0000 C CNN
	1    6550 1650
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR08
U 1 1 4DD30F78
P 5750 2450
F 0 "#PWR08" H 5750 2450 30  0001 C CNN
F 1 "GND" H 5750 2380 30  0001 C CNN
	1    5750 2450
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR09
U 1 1 4DD30F74
P 4250 2450
F 0 "#PWR09" H 4250 2450 30  0001 C CNN
F 1 "GND" H 4250 2380 30  0001 C CNN
	1    4250 2450
	1    0    0    -1  
$EndComp
$Comp
L C C3
U 1 1 4DD30F54
P 5750 2100
F 0 "C3" H 5800 2200 50  0000 L CNN
F 1 "10uF" H 5800 2000 50  0000 L CNN
	1    5750 2100
	1    0    0    -1  
$EndComp
$Comp
L C C2
U 1 1 4DD30F51
P 4250 2100
F 0 "C2" H 4300 2200 50  0000 L CNN
F 1 "10uF" H 4300 2000 50  0000 L CNN
	1    4250 2100
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR010
U 1 1 4DD30F46
P 5000 2250
F 0 "#PWR010" H 5000 2250 30  0001 C CNN
F 1 "GND" H 5000 2180 30  0001 C CNN
	1    5000 2250
	1    0    0    -1  
$EndComp
$Comp
L V7809 U1
U 1 1 4DD30F0E
P 5000 1850
F 0 "U1" H 5150 1654 60  0000 C CNN
F 1 "V7809" H 5000 2050 60  0000 C CNN
	1    5000 1850
	1    0    0    -1  
$EndComp
$Comp
L +5V #PWR011
U 1 1 4DD308A2
P 5750 3250
F 0 "#PWR011" H 5750 3340 20  0001 C CNN
F 1 "+5V" H 5750 3340 30  0000 C CNN
	1    5750 3250
	1    0    0    -1  
$EndComp
$Comp
L PWR_FLAG #FLG012
U 1 1 4DD3088D
P 5700 3850
F 0 "#FLG012" H 5700 4120 30  0001 C CNN
F 1 "PWR_FLAG" H 5700 4080 30  0000 C CNN
	1    5700 3850
	1    0    0    -1  
$EndComp
$Comp
L +5V #PWR013
U 1 1 4DD3087D
P 5450 3700
F 0 "#PWR013" H 5450 3790 20  0001 C CNN
F 1 "+5V" H 5450 3790 30  0000 C CNN
	1    5450 3700
	1    0    0    -1  
$EndComp
$Comp
L VDD #PWR014
U 1 1 4DD3074D
P 4450 2950
F 0 "#PWR014" H 4450 3050 30  0001 C CNN
F 1 "VDD" H 4450 3060 30  0000 C CNN
	1    4450 2950
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR015
U 1 1 4DD2F8D7
P 3550 5600
F 0 "#PWR015" H 3550 5600 30  0001 C CNN
F 1 "GND" H 3550 5530 30  0001 C CNN
	1    3550 5600
	1    0    0    -1  
$EndComp
$Comp
L VDD #PWR016
U 1 1 4DD2F8C8
P 2700 5300
F 0 "#PWR016" H 2700 5400 30  0001 C CNN
F 1 "VDD" H 2700 5410 30  0000 C CNN
	1    2700 5300
	1    0    0    -1  
$EndComp
$Comp
L CONN_4 P3
U 1 1 4DD2F842
P 2300 5600
F 0 "P3" V 2250 5600 50  0000 C CNN
F 1 "Display" V 2350 5600 50  0000 C CNN
	1    2300 5600
	-1   0    0    1   
$EndComp
$Comp
L GND #PWR017
U 1 1 4DD2F728
P 4600 3800
F 0 "#PWR017" H 4600 3800 30  0001 C CNN
F 1 "GND" H 4600 3730 30  0001 C CNN
	1    4600 3800
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR018
U 1 1 4DD2F63F
P 6800 3600
F 0 "#PWR018" H 6800 3600 30  0001 C CNN
F 1 "GND" H 6800 3530 30  0001 C CNN
	1    6800 3600
	1    0    0    -1  
$EndComp
$Comp
L R R1
U 1 1 4DD2F567
P 6150 3400
F 0 "R1" V 6230 3400 50  0000 C CNN
F 1 "68" V 6150 3400 50  0000 C CNN
	1    6150 3400
	0    1    1    0   
$EndComp
$Comp
L C C4
U 1 1 4DD2F558
P 6800 3250
F 0 "C4" H 6850 3350 50  0000 L CNN
F 1 "0.33uF" H 6850 3150 50  0000 L CNN
	1    6800 3250
	1    0    0    -1  
$EndComp
Text Label 5350 2950 0    60   ~ 0
RST
Text Label 2250 3800 0    60   ~ 0
RST
NoConn ~ 6450 3050
NoConn ~ 5650 3050
$Comp
L CONN_2X2 P2
U 1 1 4DD2F1DB
P 6050 3000
F 0 "P2" H 6050 3150 50  0000 C CNN
F 1 "ResetOverride" H 6060 2870 40  0000 C CNN
	1    6050 3000
	1    0    0    -1  
$EndComp
NoConn ~ 4350 3800
NoConn ~ 4350 4800
NoConn ~ 4350 4900
Text Label 2050 4700 0    60   ~ 0
DisplayTX
Text Label 2050 4600 0    60   ~ 0
DisplayRX
$Comp
L GND #PWR019
U 1 1 4DB0D0E3
P 1550 4000
F 0 "#PWR019" H 1550 4000 30  0001 C CNN
F 1 "GND" H 1550 3930 30  0001 C CNN
	1    1550 4000
	1    0    0    -1  
$EndComp
NoConn ~ 2650 3700
NoConn ~ 2650 3600
Text Label 4550 4000 0    60   ~ 0
PotentiometerD
Text Label 4550 4100 0    60   ~ 0
CurrentSenseD
Text Label 4550 4200 0    60   ~ 0
PotentiometerC
Text Label 4550 4300 0    60   ~ 0
CurrentSenseC
Text Label 4550 4400 0    60   ~ 0
PotentiometerB
Text Label 4550 4500 0    60   ~ 0
CurrentSenseB
Text Label 4550 4600 0    60   ~ 0
PotentiometerA
Text Label 4550 4700 0    60   ~ 0
CurrentSenseA
$Comp
L VAA #PWR1
U 1 1 4DAF3EF4
P 3700 1600
F 0 "#PWR1" H 3700 1660 30  0001 C CNN
F 1 "VAA" H 3700 1710 30  0000 C CNN
	1    3700 1600
	1    0    0    -1  
$EndComp
$Comp
L ARDUINO_NANO U2
U 1 1 4DAF2A1C
P 3500 4350
F 0 "U2" H 3050 5300 60  0000 C CNN
F 1 "ARDUINO_NANO" H 3500 3500 60  0000 C CNN
	1    3500 4350
	1    0    0    -1  
$EndComp
$Comp
L CONN_2 P1
U 1 1 4B885718
P 2850 2000
F 0 "P1" V 2800 2000 40  0000 C CNN
F 1 "POWER" V 2900 2000 40  0000 C CNN
	1    2850 2000
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR020
U 1 1 4B884901
P 3200 3050
F 0 "#PWR020" H 3200 3050 30  0001 C CNN
F 1 "GND" H 3200 2980 30  0001 C CNN
	1    3200 3050
	1    0    0    -1  
$EndComp
$Comp
L PWR_FLAG #FLG021
U 1 1 4B61DEC3
P 2950 2750
F 0 "#FLG021" H 2950 3020 30  0001 C CNN
F 1 "PWR_FLAG" H 2950 2980 30  0000 C CNN
	1    2950 2750
	1    0    0    -1  
$EndComp
$Comp
L PWR_FLAG #FLG022
U 1 1 4B61DE8F
P 3450 1500
F 0 "#FLG022" H 3450 1770 30  0001 C CNN
F 1 "PWR_FLAG" H 3450 1730 30  0000 C CNN
	1    3450 1500
	1    0    0    -1  
$EndComp
$Comp
L C C1
U 1 1 4B5F73D3
P 3450 2100
F 0 "C1" H 3300 2200 50  0000 L CNN
F 1 "10uF" H 3150 2000 50  0000 L CNN
	1    3450 2100
	1    0    0    -1  
$EndComp
$EndSCHEMATC
